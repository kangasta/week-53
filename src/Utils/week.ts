...
const getWeekThursdayDate = (date: Date): number =>
...
export const getWeek = (year: number, weekNumber: number): Date[] => {
  const thursday = new Date(
    getFirstThursday(year).getTime() + (weekNumber - 1) * WEEK_IN_MS
  );
  const monday = getWeekMonday(thursday);

  return [...Array(7)].map(
    (_, i): Date => new Date(monday.getTime() + i * DAY_IN_MS)
  );
};
...
export const getWeeksInYear = (year: number): number =>
  getWeekNumber(new Date(year, 11, 28));
...
export const isToday = (date: Date): boolean => {
  ...
  return ["getDate", "getMonth", "getFullYear"].every(
    (fnName) => today[fnName]() === date[fnName]()
  );
};
